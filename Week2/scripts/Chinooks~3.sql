...
select * from genre;
insert into genre (name) values('histo');
...
create or replace function get_num_artists
  return number --must return something, declare return type here
  is
    total number;
  BEGIN
...
faster retrieval of records. An index creates an enry for
each value that appears in the indexed columns(s).
*/
