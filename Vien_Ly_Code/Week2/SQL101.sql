...
-- essentially saved queries, cannot update tables via views
create view Al_Art_View as
  select album.albumid as album, artist.artistid, artist.name 
      from album 
      join artist 
      on album.artistid = artist.artistid;
...
JOIN PLAYLIST ON PLAYLIST.PLAYLISTID = PLAYLIST.PLAYLISTID
